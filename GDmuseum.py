...
# 线程安全
def LockThreadGetURL(url):
    while not islock:
        print("等待解锁")
        time.sleep(1)
    print("已经解锁，利用代理获取URL")
...
def DownImage(image_url) -> None:
    try:
        image_res = LockThreadGetURL(image_url)
...
def download_comic(comic_url, comic_name, *args, **kwargs) -> None:
    ...
    if os.path.exists(file_pth):
        return
    response = LockThreadGetURL(comic_url)
